...
namespace gazebo
{
  class PietonPlugin2 : public ModelPlugin
  {
    public: virtual void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>(
       "/" + this->model->GetName() + "/pieton_velocity2",
       1,
       boost::bind(&PietonPlugin2::OnRosMsg, this, _1),
       ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);

      // Spin up the queue helper thread.
      this->rosQueueThread = std::thread(std::bind(&PietonPlugin2::QueueThread, this));
  ...
  GZ_REGISTER_MODEL_PLUGIN(PietonPlugin2)
}
