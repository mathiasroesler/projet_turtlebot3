...
namespace gazebo
{
  class BarrierPlugin2 : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>(
       "/" + this->model->GetName() + "/barriere_position2",
       1,
       boost::bind(&BarrierPlugin2::OnRosMsg, this, _1),
       ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);

      // Spin up the queue helper thread.
      this->rosQueueThread = std::thread(std::bind(&BarrierPlugin2::QueueThread, this));
    ...
    // Sets position of the barrier
    {
      ...
      if (_position == 0)
      {
        ignition::math::Vector3<double> position_closed(-0.50, 1.0, 0.18); // Closed barrier shift
	...
	this->model->SetLinkWorldPose(pose, this->link); // Set the position of the barrier
      ...
      else if (_position == 1)
      {
        ...
        ignition::math::Vector3<double> position_opened(-0.50, 1.2, 0.18); // Opened barrier shift
        pose.Rot() = rot;
  ...
  GZ_REGISTER_MODEL_PLUGIN(BarrierPlugin2)
}
